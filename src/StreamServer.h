...
//   GET /oauth/callback -> Google sign-in redirect (see GoogleDocs.h)
...
public:
    ...
    // Session-notes link (a Google Doc attached by the sender) for the listener
    // page: injected into the served page so the button is there before play,
    // sent in the hello frame, and pushed to connected listeners on change.
    void setNotesUrl(const juce::String& u)
    {
        {
            const juce::ScopedLock sl(idLock);
            if (notesUrl == u)
                return;
            notesUrl = u;
        }
        notesVersion.fetch_add(1);
    }
    juce::String getNotesUrl() const { const juce::ScopedLock sl(idLock); return notesUrl; }

    // GET /oauth/callback?...: returns the HTTP status to answer with, html
    // receives the body. Called on the accept thread (may block briefly).
    std::function<int(const juce::String& target, juce::String& html)> oauthCallback;
...
private:
    ...
    std::vector<uint8_t> buildNotesFrame() const;
    ...
    juce::String streamId, notesUrl;
    std::atomic<int> notesVersion { 0 };
    std::vector<std::unique_ptr<Client>> clients;

    int lastListenerCount = -1;   // broadcast thread only
    int lastNotesVersion = 0;     // broadcast thread only
