...
<style>
  ...
  * { box-sizing: border-box; margin: 0; }
  ...
  /* ---- session notes: a Google Doc the sender attached ---- */
  .notes { display:none; justify-content:center; margin-top:14px; }
  .notes.show { display:flex; }
  .notes a { display:inline-flex; align-items:center; gap:11px; padding:10px 18px 10px 15px;
             border-radius:999px; border:1px solid #23232c; background:#16161c;
             color:#e8e8ec; font-size:13px; font-weight:500; text-decoration:none;
             text-align:left; transition:border-color 140ms ease; }
  .notes a:hover { border-color:#4f6bff; }
  .ndoc { width:12px; height:15px; border:1.5px solid #8a8a96; border-radius:2px;
          position:relative; flex:none; }
  .ndoc::before, .ndoc::after { content:""; position:absolute; left:2px; right:2px;
                                height:1.5px; background:#8a8a96; }
  .ndoc::before { top:4px; } .ndoc::after { top:8px; }
  .notes small { display:block; font-size:9.5px; letter-spacing:.12em; color:#5c5c68;
...
// before play) and pushes changes over the socket. Only docs.google.com
// document links are ever shown.
function setNotes(url) {
  const ok = typeof url === 'string'
          && /^https:\/\/docs\.google\.com\/document\/d\/[A-Za-z0-9_-]{10,}/.test(url);
  notesLink.href = ok ? url : '#';
  notes.classList.toggle('show', ok);
}
setNotes("__LL_NOTES__");
...
function connect() {
  ...
  ws.onmessage = async (e) => {
    if (typeof e.data === 'string') {
      ...
      if (msg.notes !== undefined) setNotes(msg.notes);
      if (msg.sampleRate) {
