...
void StreamServer::handleConnection(std::unique_ptr<juce::StreamingSocket> sock)
{
    ...
    if (path == "/" || path == "/index.html")
    {
        ...
        // The notes link is baked in so the button shows before play.
        const auto body = juce::String::fromUTF8(kListenerPage)
                              .replace("__LL_NOTES__", jsonEscape(getNotesUrl()));
        const auto response =
            "HTTP/1.1 200 OK\r\n"
            "Content-Type: text/html; charset=utf-8\r\n"
...
void StreamServer::broadcastLoop(juce::Thread& thread)
{
    while (! thread.threadShouldExit())
    {
        ...
        // Notes link changed (attached / swapped / detached): tell everyone.
        std::vector<uint8_t> notesFrame;
        if (const int nv = notesVersion.load(); nv != lastNotesVersion)
        {
            lastNotesVersion = nv;
            notesFrame = buildNotesFrame();
        }
...
juce::String StreamServer::jsonEscape(const juce::String& s)
{
    // Enough for URLs and short strings inside a JSON string literal that
    // also sits inside a <script> block (hence the escaped '<').
    return s.replace("\\", "\\\\").replace("\"", "\\\"").replace("<", "\\u003c")
            .replace("\n", "\\n").replace("\r", "\\r");
}

std::vector<uint8_t> StreamServer::buildNotesFrame() const
{
    const auto json = "{\"notes\":\"" + jsonEscape(getNotesUrl()) + "\"}";
